...
function M.bind_key(map)
  for key, value in pairs(map) do
    ...
    if not valid_mode[mode] then assert(false, 'Not a valid mode ' .. mode) end
...
function M.vertical_resize(direction)
...
local fzf_spell_replace = function(word) vim.cmd('normal! \"_ciw' .. word) end

function M.fzf_spell()
...
function M.ripgrep_fzf(query, fullscreen)
  local cmd_fmt =
  ...
  vim.fn['fzf#vim#grep'](cmd_init, 1, vim.fn['fzf#vim#with_preview'](opts),
    fullscreen)
end
